...
//NOTES: everything thats not atmega644 is standard pinout incl atmega 1284
//KLAAS: TEMP REMOVE
///#define __AVR_ATmega644A__
//#define ARDUINO_MAIN
...
// We're using the pro mini 2 pinout
#define PROMINI2_PINOUT

// Digital pins
#if defined(__AVR_ATmega324PB__)
  #define NUM_DIGITAL_PINS          (39)
#else
//TODO: ex JTAG, incl a6/a7
...
defined(__AVR_ATmega324P__) || defined(__AVR_ATmega324PA__) || defined(__AVR_ATmega644A__) || defined(__AVR_ATmega644P__)
  #define digitalPinHasPWM(p)       ((p) >= 7 && (p) <= 10)
...
#define PIN_A4 (18)
#define PIN_A5 (19)
#define PIN_A6 (30) //same as standard pinout
#define PIN_A7 (31) //same as standard pinout
...
//TODO
#define analogInputToDigitalPin(p)  ((p) <= 5 ? (p) + 14 : (p) <= 7 ? (p) + 24 : -1)
#define analogPinToChannel(p)       ((p) < NUM_ANALOG_INPUTS ? (p) : (p) >=30 ? (p)-24 : (p) >= 14 ? (p) - 14 : -1)
...
#else
  #define digitalPinToPCMSK(p)  (((p) >= 0 && (p) <= 3) || (p) == 8 || (p)==9 ? &PCMSK3 : (p) >= 4 && (p) <= 13 && (p) != 8 && (p) !=9 ? &PCMSK1 : (p) >= 14 && (p) <= 17 ? &PCMSK0 : &PCMSK2)
#endif
//attention: will return 0 for unmapped pin
#define digitalPinToPCMSKbit(p) ((p) == 0 ? 0 : (p) == 1 ? 1 : (p) == 2 ? 2 : (p) == 3 ? 3 : (p) == 4 ? 0 : (p) == 5 ? 1 : (p) == 6 ? 2 : (p) == 7 ? 3 : (p) == 8 ? 6 : (p) == 9 ? 7 : (p) == 10 ? 4 : (p) == 11 ? 5 : (p) == 12 ? 6 : (p) == 13 ? 7 : (p) == 14 ? 0 : (p) == 15 ? 1 : (p) == 16 ? 2 : (p) == 17 ? 3 : (p) == 18 ? 1 : (p) == 19 ? 0 : (p) == 20 ? 2 : (p) == 21 ? 3 : (p) == 22 ? 4 : (p) == 23 ? 5 : 0)

//same as standard pinout
#define digitalPinToPCICR(p)    (((p) >= 0 && (p) < NUM_DIGITAL_PINS) ? (&PCICR) : ((uint8_t *)0))

#define digitalPinToPCICRbit(p) ( ((p) >= 0 && (p) <= 3) || (p) == 8 || (p)==9 ? 3 : (p) >= 4 && (p) <= 13 && (p) != 8 && (p) !=9 ? 1 : (p) >= 14 && (p) <= 17 ? 0 : 2)
